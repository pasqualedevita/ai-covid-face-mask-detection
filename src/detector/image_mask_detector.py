# USAGE
# python src/detector/image_mask_detector.py --image images/pic1.jpeg
...
def load_mask_detetcor(modelDir):
    ...
    maskNet = load_model(os.path.sep.join([modelDir, file]))
    return maskNet
...
if __name__ == "__main__":
    ...
    faceNet = load_face_detector(args["face_detector"])
    maskNet = load_mask_detetcor(args["mask_detector"])
    image   = load_image(args["image"])
    ROIs    = detect_face(image, faceNet, args["confidence"])
	detect_mask(maskNet, image, ROIs)
